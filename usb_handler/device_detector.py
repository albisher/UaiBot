...
Provides functionality to detect and list USB devices in both local and remote contexts
"""
import re
import subprocess
import os
...
class USBDeviceDetector:
    ...
    def generate_remote_device_command(self, query=None):
        """
        Generate a command to detect USB devices on a remote system
        
        Args:
            query (str): The user query for context
            
        Returns:
            str: The command to run on the remote system
        """
        # Choose the appropriate command based on the query context
        if query and ("list" in query.lower() or "show" in query.lower()):
            return "ls -l /dev/tty* /dev/cu.* 2>/dev/null || ls -l /dev/ttyUSB* /dev/ttyACM* 2>/dev/null"
        elif query and ("dmesg" in query.lower() or "logs" in query.lower()):
            return "dmesg | grep -i usb | tail -20"
        else:
            # Default command - comprehensive detection that works on most systems
            return "echo '📱 USB Devices on Remote System:' && echo '' && " + \
