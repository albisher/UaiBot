...
def test_sequence_commands(uaibot: UaiBot):
    ...
    result = app.process_command("where is Kuwait")
    ...
    result = app.process_command("in that browser click the middle link")
    ...
    result = app.process_command("make that browser more focused on the text")
    ...
    result = app.process_command("increase volume to 80%")
    ...
    result = app.process_command("now in safari go to youtube")
    ...
    result = app.process_command("in there search for Quran by Al Husay")
    ...
    result = app.process_command("play that and cast it to my tv")
