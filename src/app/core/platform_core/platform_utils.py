...
def get_audio_handler() -> Optional[Any]:
    ...
    try:
        if system == 'Darwin':
            from labeeb.platform_core.mac.audio_handler import MacAudioHandler
...
def get_usb_handler() -> Optional[Any]:
    ...
    try:
        if system == 'Darwin':
            from labeeb.platform_core.mac.usb_handler import MacUSBHandler
...
def get_input_handler() -> Optional[Any]:
    ...
    try:
        if system == 'Darwin':
            from labeeb.platform_core.mac.input_handler import MacInputHandler
